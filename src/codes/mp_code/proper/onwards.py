...
class ImageProcessor:
    ...
    def generate_waypoints(self, contour_points_left, contour_points_right):
        ...
        elif self.uniform_midpoint_distance is not None:
            single_line = contour_points_left if contour_points_left is not None else contour_points_right
            is_left_line_detected = contour_points_left is not None

            for x, y in single_line:
                if is_left_line_detected:
                    midpoint_x = x + self.uniform_midpoint_distance
                else:
                    midpoint_x = x - self.uniform_midpoint_distance

                contour_points_middle.append((midpoint_x, y))
        else:
            print("Uniform midpoint distance is not yet calculated")
            return np.array([]), np.array([])

        if not contour_points_middle:
            print("No midpoints generated")
            return np.array([]), np.array([])

        contour_points_middle = np.array(contour_points_middle, dtype=float)
        if contour_points_middle.ndim < 2 or contour_points_middle.shape[1] != 2:
            print("Unexpected shape of contour points middle")
            return np.array([]), np.array([])
