...
class ImageProcessor:
    ...
    def draw_lanes(self, original_image, x_values_left, y_values_left, x_values_right, y_values_right):
        lane_image = np.copy(original_image)
        
        for i in range(len(x_values_left) - 1):
            cv2.line(lane_image, (int(x_values_left[i]), int(y_values_left[i])), (int(x_values_left[i+1]), int(y_values_left[i+1])), (0, 255, 0), 5)

        for i in range(len(x_values_right) - 1):
            cv2.line(lane_image, (int(x_values_right[i]), int(y_values_right[i])), (int(x_values_right[i+1]), int(y_values_right[i+1])), (0, 255, 0), 5)

        return lane_image

# In process_image method, after detecting lanes
